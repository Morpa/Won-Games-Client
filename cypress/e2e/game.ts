...
describe('Game Page', () => {
  ...
  it('should render game page sections', () => {
    ...
    cy.getByDataCy('game-details').within(() => {
      ...
      cy.findAllByText(/bloober team SA/i).should('have.length', 2)
      cy.findByText(/jan 27, 2021/i).should('exist')
      cy.findByRole('img', { name: /windows/i }).should('exist')
      cy.findByText(/18+/i).should('exist')
      cy.findByText('Adventure / Action / Horror').should('exist')
    })
  ...
  it('should add/remove game in cart', () => {
    ...
    cy.findAllByLabelText(/cart items/i)
      ...
      cy.getByDataCy('cart-list').within(() => {
        cy.findByRole('heading', { name: /the medium/i }).should('exist')
      })
